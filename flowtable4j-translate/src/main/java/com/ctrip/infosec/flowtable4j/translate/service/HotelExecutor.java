...
public class HotelExecutor implements Executor
{
    ...
    public void complementData(DataFact dataFact, Map data, ThreadPoolExecutor executor)
    {
        ...
        {
            logger.warn(data.get("OrderID").toString()+"酒店数据处理异常"+exp.getMessage());
        }
    }
    ...
    public void convertToFlowRuleCheckItem(DataFact dataFact, Map data, Map flowData)
    {
        ...
        {//产品信息
            ...
            {
                logger.warn("酒店"+getValue(flowData,Common.OrderID)+"解析时间格式异常"+e.getMessage());
            }
    ...
    //获取到达酒店到订单时间的差值
    public long getArrivalTimeOrderDate(DataFact dataFact,Map data)
    {
        try
        {
            logger.info(data.get("OrderID")+"获取时间的差值相关信息");
    ...
    //获取到达酒店到用户注册时间的差值
    public long getArrivalTimeSignUpDate(DataFact dataFact,Map data)
    {
        try{
            logger.info(data.get("OrderID")+"获取时间的差值相关信息");
            //订单日期
