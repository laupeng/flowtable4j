...
public class ConverterBase {
    ...
    public Map getCityAndProv(String mobilePhone)
    {
        ...
        try
        {
            ...
            String sqlCommand = "SELECT Top 1 CityName,ProvinceName FROM BaseData_MobilePhoneInfo with (nolock) WHERE MobileNumber = ?";
            mobileInfo = cardRiskDb.queryForMap(sqlCommand,new Object[]{ subMobileNum },new int[]{Types.VARCHAR});
        }catch(Exception ex)
        {
            logger.warn("get City and Province by Mobile phone error",ex);
        }
    ...
    public Map getIpCountryCity(long ipValue)
    {
        ...
        try{
            ...
            ipInfo = cardRiskDb.queryForMap(sqlCommand,new Object[]{ipValue}, new int[]{Types.BIGINT});
        }catch(Exception ex)
        {
            logger.warn("get IP country error:",ex);
        }
        return ipInfo;
    }
...
//        Map DIDInfo = null;
//        try{
//            String sqlCommand = "select Top 1 * from CacheData_DeviceIDInfo with (nolock) where Oid = ? and Payid = ? order by RecordID desc";
//            DIDInfo = flowDb.queryForMap(sqlCommand,new Object[]{orderId,orderType},new int[]{Types.VARCHAR,Types.VARCHAR});
//        }catch (Exception ex)
//        {
//            logger.warn("get DID error",ex);
//        }
//        return DIDInfo;
//    }
//
//
//    public Map getMainInfo(int orderType,long orderId)
//    {
//        Map mainInfo = null;
//        try
//        {
//            String sqlCmd = "SELECT top 1 * from InfoSecurity_MainInfo with (nolock) where OrderType = ?"
//                    +" and OrderId = ?  order by ReqID desc";
//            mainInfo = cardRiskDb.queryForMap(sqlCmd,new Object[]{orderType,orderId},new int[]{Types.INTEGER,Types.BIGINT});
//        }catch (Exception ex)
//        {
//            logger.warn("get MainInfo error",ex);
//        }
//        return mainInfo;
//    }
//
